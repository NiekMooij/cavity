...
Physics
-------
For a c-regular graph with coupling μ, the Bethe free entropy per site is:
...
def _logsumexp_neg_beta(beta: float, values: np.ndarray) -> float:
...
def estimate_phi(
    ...
    *,
    ...
    if beta == 0.0:
        ...
        delta_phi_site = _logsumexp_neg_beta(beta, f_site)

    f_edge = _batch_f_edge(population, rng, n_mc)
    if beta == 0.0:
        delta_phi_edge = -float(np.mean(f_edge))
    else:
        delta_phi_edge = _logsumexp_neg_beta(beta, f_edge)
