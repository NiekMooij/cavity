"""
Run abundance-histogram comparison using the installed `cavity` package APIs.
"""
...
def compare_abundance_histograms(
    ...
    *,
    ...
    pop_cav = CavityPopulation(degree, couplings, cfg, population=pop)
    abund_cav = pop_cav.abundances(marginal_samples, seed=cfg.seed + 122)

    pool = warm_up_pool(degree, couplings, cfg, burn_in=burn_in + int(cfg.T))
    pop_pool = CavityPopulation(degree, couplings, cfg, population=pool)
    abund_pool = pop_pool.abundances(marginal_samples, seed=cfg.seed + 123)
...
if __name__ == "__main__":
    print(f"Using cavity from: {cavity.__file__}")
    ...
    path = compare_abundance_histograms(
        ...
        out_path="abundance_histograms_installed.png",
    )
    print(f"wrote {path}")
